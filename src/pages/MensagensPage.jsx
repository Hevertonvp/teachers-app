...
export const MensagensPage = () => {
  ...
  // Secretaria, com uma escola selecionada: por padrão a mensagem vai para a diretoria daquela
  // escola, em vez do primeiro destinatário da lista.
  const diretorDaEscolaAtiva = isSecretaria(user) && activeEscolaId !== null
    ? recipients.find(recipient => recipient.escolaIds.includes(activeEscolaId))
    : null;
      ...
      {composeOpen && (
        <Modal title={form.respondendoA ? 'Responder mensagem' : 'Nova mensagem'} onClose={() => setComposeOpen(false)}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-3"><Button type="button" variant="secondary" onClick={() => setComposeOpen(false)}>Cancelar</Button><Button type="submit">Enviar mensagem</Button></div>
          </form>
        </Modal>
      )}
    </MainLayout>
